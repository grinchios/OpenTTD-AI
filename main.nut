...
function Mungo::HasMoney(money)
{
  ...
  */
  ...
  if (this.BankBalance() + (AICompany.GetMaxLoanAmount() - AICompany.GetLoanAmount()) > money) return true;
  return false;
}

function Mungo::BankBalance()
{ 
...
function Mungo::BuildAircraft(tile_1, tile_2)
{
	...
	*/
	...
	local balance = this.BankBalance();
...
function Mungo::ManageAirRoutes()
{
	...
	for (local i = list.Begin(); list.HasNext(); i = list.Next())
   {
	...
	for (local i = list.Begin(); list.HasNext(); i = list.Next()) {
		...
		AILog.Info("Station " + i + " (" + AIStation.GetLocation(i) + ") has too much cargo, adding a new vehicle for the route.");
...
function Mungo::CreateNewAirRoutes()
{
  ...
  if (ret == 0)
  {
    AILog.Info("Mungo bought a new aircraft");
  }
  else
  {
    AILog.Info("Error occured whilst buying new aircraft");
  }
}
...
function Mungo::Start()
{
	...
	AILog.Info("Welcome to Mungo. I will be building airports all day long.");
